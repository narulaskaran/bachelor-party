...
describe("requireAdmin", () => {
  ...
  // --- Global token only (partyToken=null) ---
  it("succeeds with correct global token", () => {
    process.env.ADMIN_API_TOKEN = "global-secret";
    const res = requireAdmin(makeRequest("global-secret"), null);
    expect(res).toBeNull();
  });
  ...
  it("succeeds with matching party token (even without global)", () => {
    process.env.ADMIN_API_TOKEN = "global-secret";
    const res = requireAdmin(makeRequest("party-1"), "party-1");
    expect(res).toBeNull();
  });

  it("succeeds with party token even when global is unset", () => {
    delete process.env.ADMIN_API_TOKEN;
    const res = requireAdmin(makeRequest("my-party-token"), "my-party-token");
    expect(res).toBeNull();
  });
  ...
  it("falls through to global when party token doesn't match request token", () => {
    process.env.ADMIN_API_TOKEN = "global-secret";
    const res = requireAdmin(makeRequest("global-secret"), "wrong-party");
    expect(res).toBeNull(); // falls through, global matches
  });

  it("returns 401 when party token wrong and global also doesn't match", () => {
    process.env.ADMIN_API_TOKEN = "global-secret";
    const res = requireAdmin(makeRequest("also-wrong"), "wrong-party");
  ...
  it("rejects party token with different length", () => {
    ...
    const res = requireAdmin(makeRequest("short"), "longer-partypass");
    expect(res!.status).toBe(401);
  });
