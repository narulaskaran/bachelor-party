...
No real trip details live in this repo. Hosts create a trip from the homepage
and get an organizer packet (invite URL, guest password, admin token) — no
`/admin` login. Organizers can also talk to the **admin API** (curl, CLI, or
an agent). Guests get the HTML page.
